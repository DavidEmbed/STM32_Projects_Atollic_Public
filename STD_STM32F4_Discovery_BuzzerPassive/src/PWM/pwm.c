...
void PWM_INIT(Tim_TypeDef Tim, uint32_t period)
{
...
// Generate pwm at certain frequency
void tone(uint16_t frequency){
	uint32_t pwm_period;
	uint32_t pulse;

	// Duty cicle: 50%
	pwm_period = (APB1_TIMER_TICK_FREQUENCY / frequency) - 1;	// Ticks
	PWM_INIT(TIM5_CH1, pwm_period);
	pulse = (uint32_t)(50 * (float32_t)(pwm_period) / 100.0f);
