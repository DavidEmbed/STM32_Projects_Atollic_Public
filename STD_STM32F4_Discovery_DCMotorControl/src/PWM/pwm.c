...
uint16_t		TIM_PIN[TIMn] 		= {TIM5_CH1_PIN};
GPIO_TypeDef*	TIM_GPIO_PORT[TIMn] = {TIM5_CH1_GPIO_PORT};
uint32_t		TIM_GPIO_CLK[TIMn] 	= {TIM5_CH1_GPIO_CLK};
uint8_t			TIM_PINSOURCE[TIMn]	= {TIM5_CH1_PINSOURCE};
uint8_t 		TIM_AF[TIMn] 		= {TIM5_CH1_AF};

// Variables PWM_INIT
uint32_t		TIM_CLK[TIMn] 		 = {TIM5_CH1_TIM_CLK};
TIM_TypeDef*	TIM[TIMn] 			 = {TIM5_CH1_TIM};
uint16_t		TIM_CHANNEL[TIMn] 	 = {TIM5_CH1_CHANNEL};
uint32_t		TIM_PWM_PERIOD[TIMn] = {TIM5_CH1_PWM_PERIOD};
...
// Function for send the number of timer and the position of degrees
uint8_t PWM_SERVOMOTOR(Tim_TypeDef Tim, uint16_t degree)
{
	float32_t pwm_percen;

	// If the value of degrees is between 0 and 180, do:
	if (degree >= 0 && degree <= 180)
	{
		...
		pwm_percen = 0.048 * degree + 3.3;

		// Sets TIMx-CHx PWM duty at PWM_Percen
		pulse = (uint32_t)(pwm_percen * (float32_t)(TIM_PWM_PERIOD[Tim]) / 100.0f);

		// Set the TIMx capture compare value for channel 1
		TIM_SetCompare1(TIM[Tim], pulse);
	}

	// If the value of degrees is not between 0 and 180, do:
	else
	{
		// The function return the value 1
		return 1;
	}
...
{
	// If the value of degrees is between 0 and 180, do:
	if (pwm_percen >= 0 && pwm_percen <= 100)
	{
		// Enables Capture/Compare TIMx-CHx output
		TIM_CCxCmd(TIM[Tim], TIM_CHANNEL[Tim], TIM_CCx_Enable);
	...
	// If the value of percentage is not between 0 and 100, do:
	else
	{
		// The function return the value 1
		return 1;
