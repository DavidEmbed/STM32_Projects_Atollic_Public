...
// SCLK = PA5,  MISO = PA6, MOSI = PA7, SS = PE3
...
// Writes a register of the MFRC522 through the SPI1
void MFRC522_WriteRegister(uint8_t writeAddr, uint8_t value) {
	...
	MFRC522_SendByte(writeAddr);

	// Send the data that will be written (MSB First)
...
// Reads a register of the MFRC522 through the SPI1
uint8_t MFRC522_ReadRegister(uint8_t readAddr) {
	uint8_t val;

	// Bit R/W to 1, indicate read operation (LIS302DL specification)
	...
	// Send the Address of the indexed register
	MFRC522_SendByte(readAddr);

	// Receive data that will be read (MSB First)
	// Send dummy byte (0x00): generate the SPI clock to LIS302DL
	val = MFRC522_SendByte(0x00);
...
MFRC522_Status_TypeDef MFRC522_ToCard(uint8_t command, uint8_t* sendData, uint8_t sendLen, uint8_t* backData, uint16_t* backLen) {
	MFRC522_Status_TypeDef status = MI_ERR;
	uint8_t irqEn = 0x00;
