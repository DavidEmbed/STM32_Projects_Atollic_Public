...
void SoundAnalog_GPIO_Config(void);
void SoundAnalog_ADC1_Ch12_Init(void);
uint16_t SoundAnalog_ReadValue(void);
...
int main(void) {
	...
	SoundAnalog_GPIO_Config();

	// Initializes the ADC1-Channel12 as single regular conversion mode
	SoundAnalog_ADC1_Ch12_Init();

	while (1) {

		// Do a ADC1-CH12 conversion and get the value
		SoundValue = SoundAnalog_ReadValue();

		printf("Potentiometer1 ADC value: %4u\r", SoundValue);
